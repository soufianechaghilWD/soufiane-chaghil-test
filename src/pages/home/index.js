...
class Index extends Component {
  ...
  setFilters = (filter) => {
    ...
    if (existedFil?.length === 0) newFilters.push(filter);
      ...
      if (existedFil[0]?.selected !== filter?.selected)
        existedFil[0].selected = filter?.selected;
      else {
        var idx = -1;
    ...
    for (let i = 0; i < newFilters?.length; i++) {
      ...
      var filterString =
        '{"name":"' + tmp?.name + '","type":"' + tmp?.type + '","selected":"';
      var selectedVal =
        tmp?.type === "swatch" ? swatchStr(tmp?.selected) : tmp?.selected;
              ...
              {({ loading, data }) => {
                ...
                if (filters?.length === 0) products = data?.category?.products;
                  ...
                  for (let i = 0; i < tmpPrds?.length; i++) {
                    ...
                    if (
                      filters?.every((fl) =>
                        prd?.attributes?.some((x) => x?.name === fl.name)
                      )
                    )
                      products.push(prd);
                  }
